...
switch ($action) {
    ...
    case 'delete':
        try {
            ...
            // Récupère le ticket ET son statut actuel
            $old = $db->prepare("
                SELECT a.id_ticket, t.statut AS ticket_statut, t.date_utilisation
                FROM admission a
                LEFT JOIN ticket_num t ON a.id_ticket = t.id_ticket
            ...
            $ticketRestored = false;
            $ticketValide   = false;

            if ($oldRow && $oldRow['id_ticket']) {
                // Si le ticket a été validé (statut = 'utilise'), on NE remet PAS à 'non utilise' —
                // il sert de preuve que l'admission a bien eu lieu.
                if ($oldRow['ticket_statut'] === 'utilise') {
                    $ticketValide = true;
                    // On conserve le ticket tel quel, pas de restore
                } else {
                    // Ticket non validé → on libère
                    $restore = $db->prepare("UPDATE ticket_num SET statut = 'non utilise' WHERE id_ticket = :id_ticket");
            ...
            $msg = $query->rowCount() . ' admission(s) supprimée(s).';
            if ($ticketValide) {
                $msg .= ' Ticket #' . $oldRow['id_ticket'] . ' conservé comme preuve (admission validée).';
            } elseif ($ticketRestored) {
                $msg .= ' Ticket #' . $oldRow['id_ticket'] . ' remis à disponible.';
            }

            echo json_encode([
                'success'        => true,
                'message'        => $msg,
                'ticket_valide'  => $ticketValide,
                'ticket_restored'=> $ticketRestored,
            ]);
        } catch (Exception $e) {
            echo json_encode(['success' => false, 'error' => $e->getMessage()]);
        }
