...
if ($action === 'generer_creneaux') {
    ...
    if ($medecin === '' || $date === '' || $debut === '' || $fin === '') {
        echo json_encode(['success' => false, 'message' => 'champs_manquants']); exit;
    }
    if (!is_numeric($medecin)) {
        echo json_encode(['success' => false, 'message' => 'medecin_invalide']); exit;
    }
    if ($date < date('Y-m-d'))  { echo json_encode(['success' => false, 'message' => 'date_passee']);      exit; }
    if ($debut >= $fin)          { echo json_encode(['success' => false, 'message' => 'horaires_invalides']); exit; }
    if ($duree < 5 || $duree > 240) { echo json_encode(['success' => false, 'message' => 'duree_invalide']); exit; }
    ...
    try {
        ...
        /* Vérifier que l'utilisateur est bien un médecin (id_role = 3) */
        $q = $db->prepare("SELECT id_user FROM user WHERE id_user = :m AND id_role = 3");
        $q->execute([':m' => $medecin]);
        if (!$q->fetch()) {
            $db->rollBack();
        ...
        while ($heureCourante + $duree * 60 <= $heureFinPlage) {
            ...
            /* Ignorer la plage de pause */
            if ($pauseDStamp !== null && $pauseFStamp !== null
                && $heureCourante >= $pauseDStamp && $heureCourante < $pauseFStamp) {
                $heureCourante += $duree * 60;
                continue;
            }

            $stmtCheck->execute([':m' => $medecin, ':d' => $date, ':hd' => $hd, ':hf' => $hf]);
            if ($stmtCheck->fetch()) {
                $ignores++;
                $heureCourante += $duree * 60;
                continue;
            ...
            /* Générer un id_creneau entier unique */
            $tentatives = 0;
            do {
                $idCreneau = random_int(1, 99999999);
                $stmtCheckId->execute([':id' => $idCreneau]);
                $tentatives++;
            } while ($stmtCheckId->fetch() && $tentatives < 50);

            try {
                $stmtInsert->execute([':id' => $idCreneau, ':d' => $date, ':hd' => $hd, ':hf' => $hf, ':m' => $medecin]);
                $crees++;
...
/* ═══════════════════════════════════════════════════════════════
   ACTION : rdv_du_patient
   ═══════════════════════════════════════════════════════════════ */
if ($action === 'rdv_du_patient') {
    $patient = trim($_GET['patient'] ?? '');
    if ($patient === '' || !ctype_digit($patient)) {
        echo json_encode(['success' => false, 'message' => 'patient_manquant']); exit;
    }
    try {
...
if ($action === 'marquer_a_confirmer') {
    ...
    if (!is_array($ids) || empty($ids)) {
        echo json_encode(['success' => false, 'message' => 'aucun_rdv_selectionne']); exit;
    }
    /* Forcer les ids en entiers pour la sécurité */
    $ids = array_map('intval', $ids);
    try {
        $db->beginTransaction();
        $placeholders = implode(',', array_fill(0, count($ids), '?'));
        $sql = "UPDATE rendez_vous SET statut = 'a_confirmer' WHERE id_rdv IN ($placeholders)";
        $q   = $db->prepare($sql);
        $q->execute(array_values($ids));
        $nb  = $q->rowCount();
        $db->commit();
        echo json_encode(['success' => true, 'message' => "{$nb} RDV marqué(s) à confirmer", 'nb' => $nb]);
    } catch (Exception $e) {
        if ($db->inTransaction()) $db->rollBack();
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
    }
...
if ($action === 'regenerer_factures') {
    ...
    if ($montant <= 0 || $montant > 10000) {
        echo json_encode(['success' => false, 'message' => 'montant_invalide']); exit;
    }
    try {
        $db->beginTransaction();
        $rdvs = $db->query("SELECT rv.id_rdv, rv.id_patient
                            FROM rendez_vous rv
                            LEFT JOIN facture f ON f.id_rdv = rv.id_rdv
        ...
        if (empty($rdvs)) {
            $db->rollBack();
            echo json_encode(['success' => false, 'message' => 'aucun_rdv_a_facturer']); exit;
        }

        $typeRow = $db->query("SELECT id_type FROM type_paiement LIMIT 1")->fetch(PDO::FETCH_ASSOC);
        if (!$typeRow) {
            $db->exec("INSERT INTO type_paiement (id_type, nom_type, description, montant) VALUES (1,'Especes','Paiement par defaut',0)");
            $idType = 1;
        } else {
        ...
        $stmt = $db->prepare("INSERT INTO facture
                                (id_facture, montant, statut, date_facture, id_rdv, id_type_paiement, id_patient)
                              VALUES (:idf, :m, 'impayee', CURDATE(), :rdv, :idt, :pat)");
        ...
        foreach ($rdvs as $rdv) {
            ...
            try {
                $stmt->execute([':idf' => $idFact, ':m' => $montant, ':rdv' => $rdv['id_rdv'],
                                ':idt' => $idType, ':pat' => $rdv['id_patient']]);
                $crees++;
            } catch (Exception $e) { $erreurs++; }
        }
        $db->commit();
        echo json_encode([
            'success' => true, 'message' => "{$crees} facture(s) générée(s)",
            'crees' => $crees, 'erreurs' => $erreurs, 'total' => $crees * $montant
        ]);
    } catch (Exception $e) {
        if ($db->inTransaction()) $db->rollBack();
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
...
/* ═══════════════════════════════════════════════════════════════
   ACTION : creneaux_morts
   ═══════════════════════════════════════════════════════════════ */
if ($action === 'creneaux_morts') {
    $heure = trim($_GET['heure'] ?? '');
    if ($heure === '') { echo json_encode(['success' => false, 'message' => 'heure_manquante']); exit; }
    try {
...
if ($action === 'supprimer_creneaux_morts') {
    $ids = $_POST['ids'] ?? [];
    if (!is_array($ids) || empty($ids)) {
        echo json_encode(['success' => false, 'message' => 'aucun_creneau_selectionne']); exit;
    }
    $ids = array_map('intval', $ids);
    try {
        $db->beginTransaction();
        $placeholders = implode(',', array_fill(0, count($ids), '?'));
        $sql = "DELETE FROM creneau WHERE id_creneau IN ($placeholders) AND LOWER(statut) = 'disponible'";
        $q   = $db->prepare($sql);
        $q->execute(array_values($ids));
        $nb  = $q->rowCount();
        $db->commit();
        echo json_encode(['success' => true, 'message' => "{$nb} créneau(x) supprimé(s)", 'nb' => $nb]);
    } catch (Exception $e) {
        if ($db->inTransaction()) $db->rollBack();
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
    }
...
/* ═══════════════════════════════════════════════════════════════
   ACTION : rdv_redistribuables
   ═══════════════════════════════════════════════════════════════ */
if ($action === 'rdv_redistribuables') {
    $medecin = trim($_GET['medecin'] ?? '');
    $date    = trim($_GET['date']    ?? '');
    if ($medecin === '' || $date === '') {
        echo json_encode(['success' => false, 'message' => 'parametres_manquants']); exit;
    }
    if (!ctype_digit($medecin)) {
        echo json_encode(['success' => false, 'message' => 'medecin_invalide']); exit;
    }
    try {
        ...
        $q = $db->prepare($sql);
        $q->execute([':m' => (int)$medecin, ':d' => $date]);
        echo json_encode(['success' => true, 'data' => $q->fetchAll(PDO::FETCH_ASSOC)]);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
    }
...
if ($action === 'redistribuer_rdv') {
    ...
    try {
        ...
        $stmt = $db->prepare("SELECT id_creneau FROM rendez_vous WHERE id_rdv IN ($placeholders)");
        $stmt->execute(array_values($ids));
        $creneauxALiberer = array_column($stmt->fetchAll(PDO::FETCH_ASSOC), 'id_creneau');

        $stmt = $db->prepare("UPDATE rendez_vous SET statut = 'a_reporter' WHERE id_rdv IN ($placeholders)");
        $stmt->execute(array_values($ids));
        $nbRdv = $stmt->rowCount();

        $nbCreneaux = 0;
        if (!empty($creneauxALiberer)) {
            $creneauxALiberer = array_map('intval', $creneauxALiberer);
            $ph2  = implode(',', array_fill(0, count($creneauxALiberer), '?'));
            $stmt = $db->prepare("UPDATE creneau SET statut = 'disponible' WHERE id_creneau IN ($ph2)");
        ...
        echo json_encode([
            ...
            'nb_rdv'      => $nbRdv, 'nb_creneaux' => $nbCreneaux
        ]);
    } catch (Exception $e) {
        if ($db->inTransaction()) $db->rollBack();
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
...
/* ═══════════════════════════════════════════════════════════════
   ACTION : profil_medecin
   ═══════════════════════════════════════════════════════════════ */
if ($action === 'profil_medecin') {
    $id = trim($_GET['id'] ?? '');
    if ($id === '' || !ctype_digit($id)) {
        echo json_encode(['success' => false, 'message' => 'id_manquant']); exit;
    }
    $id = (int)$id;
    try {
        /* Infos générales — vérification id_role = 3 */
        $q = $db->prepare("SELECT id_user, Nom, Prenom, Service, Email, CIN
                           FROM user WHERE id_user = :id AND id_role = 3");
        $q->execute([':id' => $id]);
        $medecin = $q->fetch(PDO::FETCH_ASSOC);
        if (!$medecin) {
            echo json_encode(['success' => false, 'message' => 'medecin_introuvable']); exit;
        }
        ...
        $taux  = $stats['nb_total'] > 0
               ? round(($stats['nb_reserves'] / $stats['nb_total']) * 100) : 0;

        /* Moyenne de la spécialité */
        $q = $db->prepare("SELECT AVG(taux_par_medecin) AS moyenne FROM (
        ...
        /* Évolution par tranches de 5 jours */
        $q = $db->prepare("SELECT
        ...
        foreach ($q->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            $evolution[] = [
                ...
                'total'   => (int)$row['total'], 'reserves' => (int)$row['reserves'], 'taux' => $tx
            ];
        }
        $evolution = array_reverse($evolution);

        /* Créneaux libres à venir */
        $q = $db->prepare("SELECT id_creneau, `$COL_DATE` AS date_creneau, heure_debut, heure_fin
                           FROM creneau
                           WHERE id_medecin = :id AND LOWER(statut) = 'disponible'
                             AND `$COL_DATE` >= CURDATE()
                             AND `$COL_DATE` <= DATE_ADD(CURDATE(), INTERVAL 7 DAY)
                           ORDER BY `$COL_DATE` ASC, heure_debut ASC LIMIT 5");
        $q->execute([':id' => $id]);
        $creneauxLibres = $q->fetchAll(PDO::FETCH_ASSOC);

        /* Suggestions */
        $suggestions = [];
        if ($taux < 15)                  $suggestions[] = ['icone'=>'megaphone','titre'=>'Promouvoir auprès des patients récents','detail'=>"Recommander {$medecin['Prenom']} aux patients ayant déjà consulté en {$medecin['Service']}."];
        if ($moyenneSpe > $taux + 15)    $suggestions[] = ['icone'=>'trending-down','titre'=>"Performance inférieure à la moyenne ({$moyenneSpe}%)","detail'=>'Les autres médecins de la spécialité {$medecin['Service']} ont un taux moyen de {$moyenneSpe}%."];
        if ($stats['nb_libres'] >= 5)    $suggestions[] = ['icone'=>'scissors','titre'=>'Réduire le nombre de créneaux','detail'=>"{$stats['nb_libres']} créneaux libres sur les 30 derniers jours."];
        if (count($creneauxLibres) > 0)  $suggestions[] = ['icone'=>'star','titre'=>"Mettre en avant sur la page d'accueil",'detail'=>'Afficher ses prochains créneaux libres en bannière du portail patient.'];
        if (empty($suggestions))         $suggestions[] = ['icone'=>'check','titre'=>'Aucune action urgente','detail'=>'Le profil semble équilibré, surveiller à nouveau dans 7 jours.'];

        echo json_encode([
            'success'         => true,
            'medecin'         => $medecin,
            'stats'           => ['nb_total'=>(int)$stats['nb_total'],'nb_reserves'=>(int)$stats['nb_reserves'],'nb_libres'=>(int)$stats['nb_libres'],'taux'=>$taux,'moyenne_spe'=>$moyenneSpe],
            'evolution'       => $evolution,
...
function pushAlerte(array &$alertes, string $niveau, string $categorie,
    ...
    $alertes[] = ['niveau'=>$niveau,'categorie'=>$categorie,'titre'=>$titre,
                  'detail'=>$detail,'action'=>$action,'payload'=>$payload,
                  'timestamp'=>date('Y-m-d H:i:s')];
}
function executer(string $nom, callable $fn, array &$erreurs): void {
    try { $fn(); }
    catch (Exception $e) { $erreurs[] = ['detecteur'=>$nom,'erreur'=>$e->getMessage()]; }
}

/* 1. Surcharge médecin */
executer('surcharge_medecin', function () use ($db, $COL_DATE, &$alertes) {
    $sql = "SELECT c.id_medecin, c.`$COL_DATE` AS jour,
                   CONCAT(COALESCE(u.Prenom,''),' ',COALESCE(u.Nom,'')) AS medecin,
                   u.Service AS specialite, COUNT(*) AS nb_reserves
    ...
    foreach ($db->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $r) {
        ...
        pushAlerte($alertes,'critique','Surcharge médecin',
            ...
            ['type'=>'redistribuer_rdv','id_medecin'=>(int)$r['id_medecin'],
             'medecin_nom'=>trim($r['medecin']),'date'=>$r['jour'],'nb_reserves'=>(int)$r['nb_reserves']]);
    }
}, $erreurs);

/* 2. Patient à risque */
executer('patient_a_risque', function () use ($db, &$alertes) {
    $sql = "SELECT rv.id_patient,
                   CONCAT(COALESCE(u.Prenom,''),' ',COALESCE(u.Nom,'')) AS patient,
                   COUNT(*) AS nb_annul,
            ...
            GROUP BY rv.id_patient HAVING nb_annul >= 2
    ...
    foreach ($db->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $r) {
        ...
        pushAlerte($alertes,'attention','Comportement patient',
            ...
            ['type'=>'marquer_a_confirmer','id_patient'=>(int)$r['id_patient'],
             'patient_nom'=>$nom,'nb_annul'=>(int)$r['nb_annul']]);
    }
}, $erreurs);

/* 3. Engorgement spécialité */
executer('engorgement_specialite', function () use ($db, $COL_DATE, &$alertes) {
    $sql = "SELECT u.Service AS specialite,
                   MIN(c.`$COL_DATE`) AS prochain_dispo, COUNT(*) AS nb_dispo_total
            FROM creneau c JOIN user u ON u.id_user = c.id_medecin
            WHERE LOWER(c.statut) = 'disponible' AND c.`$COL_DATE` >= CURDATE()
              AND u.Service IS NOT NULL AND u.Service <> ''
            GROUP BY u.Service
            HAVING prochain_dispo > DATE_ADD(CURDATE(), INTERVAL 7 DAY) OR nb_dispo_total < 3
            LIMIT 3";
    foreach ($db->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $r) {
        $delai  = $r['prochain_dispo'] ? (int)((strtotime($r['prochain_dispo'])-time())/86400) : null;
        $detail = $delai !== null
...
executer('medecin_sous_utilise', function () use ($db, $COL_DATE, &$alertes) {
    ...
    foreach ($db->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $r) {
        ...
        pushAlerte($alertes,'optimisation','Sous-utilisation',
            ...
            ['type'=>'voir_profil_medecin','id_medecin'=>(int)$r['id_medecin'],
             'medecin_nom'=>trim($r['medecin']),'specialite'=>$r['specialite'],
             'taux'=>$taux,'nb_total'=>(int)$r['nb_total'],'nb_reserves'=>(int)$r['nb_reserves']]);
    }
}, $erreurs);

/* 5. Créneau mort */
executer('creneau_mort', function () use ($db, $COL_DATE, &$alertes) {
    $sql = "SELECT TIME_FORMAT(c.heure_debut,'%H:%i') AS heure,
                   COUNT(*) AS nb_total,
                   SUM(CASE WHEN LOWER(c.statut)='reserve' THEN 1 ELSE 0 END) AS nb_reserves
            FROM creneau c
    ...
    foreach ($db->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $r) {
        pushAlerte($alertes,'optimisation','Créneau mort',
            ...
            ['type'=>'supprimer_creneaux_morts','heure'=>$r['heure'],'nb_total'=>(int)$r['nb_total']]);
    }
}, $erreurs);
...
executer('tendance_saisonniere', function () use ($db, &$alertes) {
    $sql = "SELECT u.Service AS specialite,
                   SUM(CASE WHEN rv.date_demande >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) THEN 1 ELSE 0 END) AS recent,
                   SUM(CASE WHEN rv.date_demande >= DATE_SUB(CURDATE(), INTERVAL 60 DAY)
                             AND rv.date_demande <  DATE_SUB(CURDATE(), INTERVAL 30 DAY) THEN 1 ELSE 0 END) AS precedent
            ...
            GROUP BY u.Service HAVING precedent >= 3";
    foreach ($db->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $r) {
        ...
        if (abs($variation) >= 25) {
            $sens = $variation > 0 ? '+' : '';
            $pct  = round($variation);
            $reco = $variation > 0 ? "Ouvrir des créneaux supplémentaires" : "Réduire l'offre · proposer aux confrères";
            pushAlerte($alertes,'prediction','Tendance saisonnière',
                ...
                $variation > 0 ? ['type'=>'generer_creneaux','specialite'=>$r['specialite']] : null);
        }
    }
}, $erreurs);
...
executer('rdv_fantome', function () use ($db, &$alertes) {
    $nb = (int)$db->query("SELECT COUNT(*) FROM rendez_vous rv
                           LEFT JOIN facture f ON f.id_rdv = rv.id_rdv
                           WHERE LOWER(rv.statut) = 'confirme' AND f.id_facture IS NULL")->fetchColumn();
    if ($nb > 0) {
...
$score = max(0, min(100,
    100 - ($compteurs['critique']*15) - ($compteurs['attention']*7)
