...
<title>JumeauNum – Gestion des Admissions</title>
...
.badge-urgence   { background: var(--red-lt);     color: var(--red); }
.badge-normal    { background: var(--green-lt);   color: var(--green); }
.badge-transfert { background: var(--amber-lt);   color: var(--amber); }
.badge-autre     { background: var(--primary-lt); color: var(--primary); }
...
.ticket-info { background: var(--primary-lt); border: 1px solid #bfdbfe; border-radius: 7px; padding: 8px 10px; font-size: .75rem; color: var(--primary); margin-top: 4px; display: none; }
.ticket-info.visible { display: block; }
.section-title { font-size: 1.05rem; font-weight: 700; color: var(--text); margin: 28px 0 12px; padding-bottom: 8px; border-bottom: 2px solid var(--border); display: flex; align-items: center; gap: 8px; }
.section-title svg { width: 15px; height: 15px; color: var(--primary); }
.right-panel-salle { display: flex; flex-direction: column; gap: 14px; }
...
<div class="layout">
  ...
  <!-- MAIN -->
  <main>
    <div class="page-title">Gestion des Admissions</div>
    <div class="page-sub">Consultez, gérez et suivez toutes les admissions de l'hôpital.</div>
    ...
    <div class="stat-grid">
      <div class="stat-card">
        <span class="stat-badge badge-blue" id="stat-badge-total">0</span>
        <div class="stat-icon blue"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 12h4l3-8 4 16 3-8h4"/></svg></div>
        <div class="stat-label">Total admissions</div>
        <div class="stat-value" id="stat-total">0</div>
        <div class="stat-sub">Toutes entrées</div>
      </div>
      <div class="stat-card">
        <span class="stat-badge badge-red" id="stat-badge-urgence">0</span>
        ...
        <div class="stat-value" id="stat-urgence">0</div>
        <div class="stat-sub">Mode urgence</div>
      </div>
      <div class="stat-card">
        <span class="stat-badge badge-green" id="stat-badge-normal">0</span>
        ...
        <div class="stat-sub">Mode normal</div>
      </div>
      <div class="stat-card">
        <span class="stat-badge badge-amber" id="stat-badge-transfert">0</span>
        <div class="stat-icon ambr"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg></div>
        <div class="stat-label">Transferts</div>
        <div class="stat-value" id="stat-transfert">0</div>
        <div class="stat-sub">Mode transfert</div>
      </div>
    </div>
    ...
    <div class="twin-zone">
      ...
      <div class="table-section">
        <div class="toolbar">
          ...
          <div class="toolbar-right">
            <span class="filter-label">Mode d'entrée</span>
        ...
        <div class="search-row">
          <div class="search-wrap">
            ...
            <input type="text" id="searchInput" placeholder="Rechercher par ID, ticket, mode..." oninput="renderTable()"/>
          </div>
        </div>
        <div class="table-wrap">
          ...
          <div class="pagination">
            <div class="count" id="paginCount">—</div>
            <button class="page-btn" onclick="prevPage()">‹</button>
            <span id="pageInfo" style="font-size:.78rem;color:var(--muted);padding:0 4px"></span>
      ...
      <div class="right-panel">
        <div class="panel-card">
          <div class="panel-title" id="formTitle">Nouvelle Admission</div>

          <div class="form-group">
            <label class="form-label" for="a_id">ID Admission *</label>
            <div style="display:flex;gap:6px;align-items:flex-start">
              <input class="form-control" type="number" id="a_id" placeholder="Ex: 1001" min="1" max="99999999" step="1" style="flex:1"/>
              <button type="button" style="width:34px;height:34px;flex-shrink:0;border-radius:7px;border:1px solid var(--border);background:var(--surface2);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:16px;transition:background .15s" onclick="rollDiceAdmission()" title="Generer un ID aleatoire disponible" onmouseover="this.style.background='var(--primary-lt)'" onmouseout="this.style.background='var(--surface2)'">&#127922;</button>
            </div>
            <div style="font-size:.65rem;color:var(--muted);margin-top:3px">&#127922; = ID aleatoire unique garanti</div>
            <div class="field-msg" id="msg_a_id"></div>
          </div>

          <div class="form-group">
            <label class="form-label" for="a_date">Date d'arrivée réelle *</label>
            <input class="form-control" type="date" id="a_date"/>
            <div class="field-msg" id="msg_a_date"></div>
          </div>

          <div class="form-group">
            <label class="form-label" for="a_mode">Mode d'entrée *</label>
            <select class="form-control" id="a_mode">
            ...
            <div class="field-msg" id="msg_a_mode"></div>
          </div>

          <div class="form-group">
            <label class="form-label" for="a_ticket">Ticket (optionnel)</label>
            <select class="form-control" id="a_ticket" onchange="showTicketInfo()">
              <option value="">Chargement des tickets...</option>
            </select>
            <div class="field-msg" id="msg_a_ticket"></div>
            <div class="ticket-info" id="ticketInfo">
              <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="display:inline;margin-right:4px"><path d="M5 13l4 4L19 7"/></svg>
              Ticket selectionne. Il sera marque comme <strong>utilise</strong> a la validation.
            </div>
          </div>

          <div class="form-group">
            <label class="form-label" for="a_salle">Salle (optionnel)</label>
            <select class="form-control" id="a_salle">
              <option value="">Chargement des salles...</option>
            </select>
          </div>

          <div class="form-group">
            <label class="form-label" for="a_patient">Patient (optionnel)</label>
            <select class="form-control" id="a_patient">
              <option value="">Chargement des patients...</option>
            </select>
            <div style="font-size:.65rem;color:var(--muted);margin-top:3px">Un email de confirmation sera envoyé au patient sélectionné.</div>
          </div>
          ...
          <button class="btn btn-primary btn-block" onclick="submitAdmission()">
            <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>
            <span id="btnSubmitLabel">Ajouter Admission</span>
          </button>
        ...
        <!-- INFO PANEL -->
        <div class="panel-card">
          <div class="panel-title">À propos des tickets</div>
          <div style="font-size:.78rem;color:var(--muted);line-height:1.6">
            <p>Le dropdown affiche uniquement les tickets avec le statut <strong style="color:var(--green)">non utilisé</strong>.</p>
            <br/>
            <p>Lors de l'ajout d'une admission, le ticket sélectionné est automatiquement marqué comme <strong style="color:var(--primary)">utilisé</strong>.</p>
            <br/>
            <p>En cas de suppression d'une admission, le ticket associé est remis à <strong style="color:var(--green)">non utilisé</strong>.</p>
          </div>
          <div onclick="loadTickets()" style="font-size:.75rem;color:var(--primary);cursor:pointer;margin-top:10px;display:inline-flex;align-items:center;gap:3px">
    ...
    <!-- Stats salles -->
    <div class="stat-grid" style="margin-top:0;margin-bottom:18px">
      <div class="stat-card">
        <span class="stat-badge badge-blue" id="stat-badge-salles">0</span>
        <div class="stat-icon blue"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg></div>
        <div class="stat-label">Total salles</div>
        <div class="stat-value" id="stat-total-salles">0</div>
      ...
      <div class="stat-card">
        ...
        <div class="stat-value" id="stat-dispo-salles">0</div>
        <div class="stat-sub">Prêtes à l'usage</div>
      </div>
      <div class="stat-card">
        <span class="stat-badge badge-red">—</span>
        <div class="stat-icon pink"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M4.93 4.93l14.14 14.14"/></svg></div>
        <div class="stat-label">Non disponibles</div>
        <div class="stat-value" id="stat-indispo-salles">0</div>
        <div class="stat-sub">Occupées / fermées</div>
      </div>
      <div class="stat-card">
        <span class="stat-badge badge-amber">—</span>
        <div class="stat-icon ambr"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="9" cy="7" r="3"/><path d="M2 19c0-3 3-5 7-5s7 2 7 5"/></svg></div>
        <div class="stat-label">Avec médecin assigné</div>
        <div class="stat-value">—</div>
        <div class="stat-sub">Médecin référent</div>
      </div>
    </div>

    <!-- Tableau + formulaire salles -->
    <div class="twin-zone">

      <!-- Tableau salles -->
      <div class="table-section">
        <div class="toolbar">
          <button class="btn btn-outline" onclick="exportSalleCSV()">
            <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 10v6M9 13l3 3 3-3M5 17a2 2 0 01-2-2V7a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2"/></svg>
            Exporter CSV
          </button>
          <div class="toolbar-right">
            <span class="filter-label">Statut</span>
            <select class="filter-select" id="filterSalleStatut" onchange="renderSalleTable()">
              <option value="">Tous</option>
        ...
        <div class="search-row">
          <div class="search-wrap">
            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
            <input type="text" id="salleSearch" placeholder="Rechercher par ID, numéro, médecin..." oninput="renderSalleTable()"/>
          </div>
        </div>
        <div class="table-wrap">
          <table>
          ...
          <div class="pagination">
            <div class="count" id="sallePaginCount">—</div>
            <button class="page-btn" onclick="prevPageS()">‹</button>
            <span id="sallePageInfo" style="font-size:.78rem;color:var(--muted);padding:0 4px"></span>
            <button class="page-btn" onclick="nextPageS()">›</button>
          </div>
        </div>
      </div>

      <!-- Formulaire salle -->
      <div class="right-panel-salle">
        <div class="panel-card">
          <div class="panel-title" id="salleFormTitle">Nouvelle Salle</div>

          <!-- ID Salle avec de aleatoire -->
          <div class="form-group">
            <label class="form-label" for="s_id">ID Salle *</label>
            <div style="display:flex;gap:6px;align-items:flex-start">
              <input class="form-control" type="number" id="s_id" placeholder="Ex: 101" min="1" max="99999999" step="1" style="flex:1"/>
              <button type="button" style="width:34px;height:34px;flex-shrink:0;border-radius:7px;border:1px solid var(--border);background:var(--surface2);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:16px;transition:background .15s" onclick="rollDiceSalle()" title="Generer un ID aleatoire disponible" onmouseover="this.style.background='var(--primary-lt)'" onmouseout="this.style.background='var(--surface2)'">&#127922;</button>
            </div>
            <div style="font-size:.65rem;color:var(--muted);margin-top:3px">&#127922; = ID aleatoire unique garanti</div>
            <div class="field-msg" id="msg_s_id"></div>
          </div>

          <!-- Numero (INT : chiffres uniquement) -->
          <div class="form-group">
            <label class="form-label" for="s_numero">Numero de salle *</label>
            <input class="form-control" type="number" id="s_numero" placeholder="Ex: 101" min="1" step="1"/>
            <div class="field-msg" id="msg_s_numero"></div>
          </div>

          <!-- Statut -->
          <div class="form-group">
            <label class="form-label" for="s_statut">Statut *</label>
            <select class="form-control" id="s_statut">
              <option value="">Selectionner...</option>
              <option value="disponible">Disponible</option>
              <option value="non disponible">Non disponible</option>
            </select>
            <div class="field-msg" id="msg_s_statut"></div>
          </div>

          <!-- Medecin (id_role = 3) -->
          <div class="form-group">
            <label class="form-label" for="s_medecin">Medecin assigne (optionnel)</label>
            <select class="form-control" id="s_medecin">
              <option value="">Chargement des medecins...</option>
            </select>
          </div>

          <p id="msg_salle_global"  style="font-size:.72rem;color:var(--red);  display:none;margin-bottom:6px"></p>
          <p id="msg_salle_success" style="font-size:.72rem;color:var(--green);display:none;margin-bottom:6px"></p>

          <button class="btn btn-primary btn-block" onclick="submitSalle()">
            <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>
            <span id="salleBtnLabel">Ajouter Salle</span>
          </button>
          <button class="btn btn-ghost btn-block" style="margin-top:5px" onclick="resetSalleForm()">Réinitialiser</button>
        </div>
      </div>

    </div><!-- /twin-zone salles -->
...
<!--
  ...
  Les deux fichiers sont dans le même dossier :
      vue/backoffice/index.html
      vue/backoffice/admission.js
  Donc src="admission.js" suffit (chemin relatif).
  L'attribut "defer" garantit que le script s'exécute
  seulement après que tout le HTML soit chargé.
-->
