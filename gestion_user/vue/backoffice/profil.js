...
async function saveProfil() {
  clearErrors(['p_nom', 'p_prenom', 'p_email', 'p_username', 'p_cin']);
  ...
  if (!username) {
    showErr('p_username', "Le nom d'utilisateur est obligatoire."); ok = false;
  } else if ((username.match(/[a-zA-ZÀ-ÿ]/g) || []).length < 2) {
    showErr('p_username', "Le nom d'utilisateur doit contenir au moins 2 lettres."); ok = false;
  } else {
    try {
      const r = await fetch(CRUD_USER_P + `?action=checkUsername&username=${encodeURIComponent(username)}&exclude_id=${SESSION_USER_ID}`);
      const j = await r.json();
      if (j.success && j.exists) {
        showErr('p_username', "Ce nom d'utilisateur est déjà pris par un autre compte."); ok = false;
      }
    } catch (e) {}
  }
