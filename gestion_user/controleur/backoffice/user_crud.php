...
switch ($action) {
    ...
    case 'checkUsername':
        try {
            ...
            if ($excludeId) {
                $q = $db->prepare("SELECT COUNT(*) as cnt FROM user WHERE username = :username AND id_user != :id");
                $q->execute(['username' => $_GET['username'] ?? '', 'id' => $excludeId]);
            } else {
                $q = $db->prepare("SELECT COUNT(*) as cnt FROM user WHERE username = :username");
                $q->execute(['username' => $_GET['username'] ?? '']);
            }
    ...
    case 'signup':
        try {
            ...
            $chkUser = $db->prepare("SELECT COUNT(*) as cnt FROM user WHERE username = :username");
            $chkUser->execute(['username' => $username]);
            if ((int)$chkUser->fetch()['cnt'] > 0) {
            ...
            $user = new User(
                $newId, $nom, $prenom, $email, $username,
                password_hash($mdp, PASSWORD_BCRYPT),
                'actif', $cin, $service, 1
            );

            $sql = "INSERT INTO user (id_user, nom, prenom, email, mot_de_passe, statut_cmpt, cin, service, id_role, username)
                    VALUES (:id_user, :nom, :prenom, :email, :mot_de_passe, :statut_cmpt, :cin, :service, :id_role, :username)";
            $q = $db->prepare($sql);
            $q->execute([
    ...
    case 'getProfil':
        try {
            $db = config::getConnexion();
            $q  = $db->prepare("SELECT id_user, nom, prenom, email, username, statut_cmpt, cin, service, id_role FROM user WHERE id_user = :id");
    ...
    case 'updateProfil':
        try {
            ...
            // Vérifier username unique (exclure l'utilisateur courant)
            $chkUser = $db->prepare("SELECT COUNT(*) as cnt FROM user WHERE username = :username AND id_user != :id");
            $chkUser->execute(['username' => $_POST['username'] ?? '', 'id' => $idUser]);
            if ((int)$chkUser->fetch()['cnt'] > 0) {
                echo json_encode(['success' => false, 'error' => 'Ce nom d\'utilisateur est déjà pris par un autre compte.']);
                break;
            }
            ...
            $sql = "UPDATE user SET nom=:nom, prenom=:prenom, email=:email, username=:username, cin=:cin, service=:service WHERE id_user=:id";
            $q   = $db->prepare($sql);
            $q->execute([
                'nom'      => trim($_POST['nom']      ?? ''),
                'prenom'   => trim($_POST['prenom']   ?? ''),
    ...
    case 'getAllUsers':
        try {
            ...
            $role = $_GET['role'] ?? '';
            if ($role !== '') {
                $q = $db->prepare("SELECT id_user, nom, prenom, email, username, cin, service, statut_cmpt, id_role FROM user WHERE id_role = :role ORDER BY nom");
                $q->execute(['role' => intval($role)]);
            } else {
                $q = $db->prepare("SELECT id_user, nom, prenom, email, username, cin, service, statut_cmpt, id_role FROM user ORDER BY nom");
                $q->execute();
            }
